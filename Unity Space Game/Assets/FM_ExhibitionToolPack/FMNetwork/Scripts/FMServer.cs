...
public class FMServer : MonoBehaviour
{
    ...
    public void Action_CheckClientStatus(string _ip)
    {
        ...
        if (!IsExistedClient)
        {
            ...
            lock (_asyncLockConnectedClient) _appendQueueConnectedClient.Enqueue(NewClient.IP);
        }
    }
    ...
    private Queue<string> _appendQueueConnectedClient = new Queue<string>();
    private Queue<string> _appendQueueDisconnectedClient = new Queue<string>();
    private object _asyncLockConnectedClient = new object();
    private object _asyncLockDisconnectedClient = new object();
    ...
    IEnumerator NetworkServerStart()
    {
        ...
        while (!stop)
        {
            ...
            while(_appendQueueConnectedClient.Count > 0)
            {
                ...
                lock (_asyncLockConnectedClient) connectedClient = _appendQueueConnectedClient.Dequeue();
                Manager.OnClientConnectedEvent.Invoke(connectedClient);
            }
            while(_appendQueueDisconnectedClient.Count > 0)
            {
                string disconnectedClient = "";
                lock (_asyncLockDisconnectedClient) disconnectedClient = _appendQueueDisconnectedClient.Dequeue();
                Manager.OnClientDisconnectedEvent.Invoke(disconnectedClient);
            }
            #endregion
    ...
    void Sender()
    {
        ...
        for (int i = ConnectionCount - 1; i >= 0; i--)
        {
            ...
            if (_active == false)
            {
                ...
                lock (_asyncLockDisconnectedClient) _appendQueueDisconnectedClient.Enqueue(ConnectedClients[i].IP);
