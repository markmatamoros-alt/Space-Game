...
public class FMNetworkManager : MonoBehaviour
{
    ...
    {
        OnClientConnectedEvent.AddListener(OnClientConnected);
        OnClientDisconnectedEvent.AddListener(OnClientDisconnected);
        OnFoundServerEvent.AddListener(OnFoundServer);
        OnLostServerEvent.AddListener(OnLostServer);
    }
    private void OnDisable()
    {
        if(NetworkType == FMNetworkType.Server)
        {
            ...
            {
                foreach (FMServer.ConnectedClient cc in Server.ConnectedClients) OnClientDisconnectedEvent.Invoke(cc.IP);
            }
        }
        else
        {
            if (Client.IsConnected && Client.FoundServer) OnLostServerEvent.Invoke(Client.ServerIP);
        }
        OnClientConnectedEvent.RemoveListener(OnClientConnected);
        OnClientDisconnectedEvent.RemoveListener(OnClientDisconnected);
        OnFoundServerEvent.RemoveListener(OnFoundServer);
        OnLostServerEvent.RemoveListener(OnLostServer);
    }
