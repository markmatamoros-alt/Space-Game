    ...
    IEnumerator NetworkClientStart()
    {
        ...
        while (!stop)
        {
            ...
            if(IsConnected != connected)
            {
                if (connected)
                {
                    Manager.OnFoundServerEvent.Invoke(ServerIP);
                }
                else
                {
                    Manager.OnLostServerEvent.Invoke(ServerIP);
                }
        ...
        try
        {
            ...
            else
            {
                    ...
                    while (_appendQueueSendPacket.Count > 0 && k < 30)
                    {
                        ...
                        if (ForceBroadcast)
                        {
                            ...
                            else
                            {
                                ...
                                _packet.SendByte[1] = 1;//when broadcast mode enabled, force the send type to server, then it won't send twice to others
                ...
                else
                {
                    ...
                    {
                        //check connection: minimum 2000ms
                        Client.Send(RequestData, RequestData.Length, new IPEndPoint(ForceBroadcast ? IPAddress.Broadcast : IPAddress.Parse(ServerIP), ServerListenPort));
                        LastSentTimeMS = Environment.TickCount;
                    }
                }
            }
